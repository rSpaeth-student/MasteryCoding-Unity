...
public class Player : MonoBehaviour
{
    ...
    {
        if (TurnStateMachine.TurnState == TurnStateMachine.State.GameOver) return;
        FindAnyObjectByType<Enemy>().StartTurn();
    }
    public int Health
    {
        get => health;
    ...
    public void DrawCard()
    {
        ...
        else
        {
            ...
            Card card = hand.NewBoardItem(cardPrefab).GetComponent<Card>();
    ...
    {
        //Debug.Log(TurnStateMachine.TurnState);
        if (TurnStateMachine.TurnState != TurnStateMachine.State.PlayerTurn) return;
        if (!card) return;
    ...
    IEnumerator ResolveCardEffects(Card_SO cardType, Creature targetCreature)
    {
        ...
        foreach (CardEffect effect in cardType.cardEffects)
        {
            switch (effect.targetType)
            {
                ...
                case CardEffect.TargetType.AllCreatures:
                    {
                        //Debug.Log("Called in Player script");
                        effect.ResolveEffect(FindObjectsByType<Creature>(FindObjectsSortMode.None));
                        break;
                    }
            }
        ...
        FindAnyObjectByType<Enemy>().PostCardPlayed();
